...
const doctors = [
  ...
  ...Array(8)
    .fill(1)
    .map((_, i) => ({
      name: `Dr. ${['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Miller', 'Davis', 'Wilson', 'Taylor'][i]}`,
      specialty: ['Cardiologist', 'Neurologist', 'Pediatrician'][i % 3],
      id: `${202545 + i + 1}`,
      email: `doctor${i + 2}@gmail.com`,
...
export default function DoctorChatInterface() {
  ...
  const DoctorList = () => (
    <div className='space-y-2'>
      <h2 className='text-[18px] font-semibold mb-4 text-[#06688E]'>Patient Medical History</h2>
      {doctors.map((doc, index) => (
        <div
          key={index}
          className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer hover:bg-[#E3F7FF] transition-colors ${selectedDoctor.id === doc.id ? 'bg-[#E3F7FF]' : ''}`}
          onClick={() => setSelectedDoctor(doc)}
        >
          <img src={doc.img} alt={doc.name} className='w-[40px] h-[40px] rounded-full object-cover' />
          <div className='flex-1 min-w-0'>
            <p className='font-semibold text-[16px] truncate text-[#455468]'>{doc.name}</p>
            <p className='text-[16px] text-[#455468] truncate'>{doc.specialty}</p>
          </div>
  ...
  const DoctorInfo = () => (
    <>
      <h2 className='text-[18px] font-semibold mb-4 text-[#06688E]'>About Doctor</h2>
      <div className='mb-6 border-b border-[#E9EFF6] pb-4'>
        <div className='flex items-center gap-3'>
          ...
          <div>
            ...
            <p className='text-[18px] text-[#455468]'>{selectedDoctor.specialty}</p>
          </div>
        </div>
      </div>
